...
/**
 * This is the model class for table "travel_seat".
 *
 * The followings are the available columns in table 'travel_seat':
 * @property integer $id
 ...
 */
class TravelSeat extends MyCActiveRecord
{
	/**
	 * Returns the static model of the specified AR class.
	 * @param string $className active record class name.
	 * @return TravelSeat the static model class
	 */
	...
	public function rules()
	{
		...
		return array(
			array('keterangan, jumlah', 'required'),
			array('jumlah', 'numerical', 'integerOnly'=>true),
			array('keterangan', 'length', 'max'=>30),
			array('gambar', 'length', 'max'=>150),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, keterangan, jumlah, gambar', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'travels' => array(self::HAS_MANY, 'Travel', 'id_travel_seat'),
		);
	}
	...
	public function attributeLabels()
	{
        ...
        public function dropdownModel(){
            $results=Yii::app()->db->createCommand()->select()
                    ->from($this->tableName())
                    ->queryAll();
            $data=array(''=>'- Pilih Type Seat -');
            foreach($results as $result){
                $data[$result['id']]=$result['keterangan']." - ".$result['jumlah'];
            }
            return $data;
        }
		...
		if($searchParam==null){
                ...
                foreach($searchParam as $column=>$value){
                    ...
                    if(strpos($column, '.id')>0 || substr($column, 0,2)=='id'){
                        ...
                        $sql->andWhere("travel_seat.$column like '%$value%'");
                }
                $counterSql=  clone $sql;
                $sql->select("travel_seat.*");
                ...
                $data->setTotalItemCount($jumlah['jumlah']);
                                $data->setSort(array('attributes'=>array('id','keterangan','jumlah','gambar')));
