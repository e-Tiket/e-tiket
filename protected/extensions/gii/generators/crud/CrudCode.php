...
/**
 * @return CModule Description mbuhlah
 */
	...
	public function generateActiveField($modelClass,$column,$options=array())
	{
            $optionStr='array(';
            $optionStrSort='';
            foreach($options as $key=>$option){
                $optionStr.="'$key'=>'$option',";
                $optionStrSort.="'$key'=>'$option',";
            }
            $optionStr.=')';
		if($column->type==='boolean')
			return "\$form->checkBox(\$model,'{$column->name}',{$optionStr})";
		else if(stripos($column->dbType,'text')!==false)
			return "\$form->textArea(\$model,'{$column->name}',array('rows'=>6, 'cols'=>50,{$optionStrSort}))";
		else
		{
			if(preg_match('/^(password|pass|passwd|passcode)$/i',$column->name))
			...
			if($column->type!=='string' || $column->size===null)
				return "\$form->{$inputField}(\$model,'{$column->name}',{$optionStr})";
			else
			{
				if(($size=$maxLength=$column->size)>60)
					$size=60;
				return "\$form->{$inputField}(\$model,'{$column->name}',array('size'=>$size,'maxlength'=>$maxLength,{$optionStrSort}))";
			}
		}
	}
