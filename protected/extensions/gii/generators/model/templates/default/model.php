...
<?php endforeach; ?>
        ...
        public function dropdownModel(){
            $results=Yii::app()->db->createCommand()->select()
                    ->from($this->tableName())
                    ->queryAll();
            $data=array(''=>'- Pilih <?php echo $this->class2name($this->modelClass);?> -');
		...
		if($searchParam==null){
                ...
                foreach($searchParam as $column=>$value){
                    ...
                    if(strpos($column, '.id')>0 || substr($column, 0,2)=='id'){
                        ...
                        $sql->andWhere("<?php echo $tableName?>.$column like '%$value%'");
                }
                $counterSql=  clone $sql;
                $sql->select("<?php echo $tableName?>.*");
                ...
                ?>
                $data->setSort(array('attributes'=>array(<?php echo $columnStr?>)));
