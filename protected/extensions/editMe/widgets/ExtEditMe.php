...
class ExtEditMe extends CInputWidget {
	...
	public function run() {
		...
		Yii::app()->clientScript -> registerScriptFile(self::$_ckeAssetUrl . '/editMe.' . md5(self::$_ckeAssetUrl) . '.min.js',CClientScript::POS_END);
		...
		if (empty($this -> inlineId)) {
			...
			Yii::app() -> clientScript -> registerScript('editMe_' . $this -> htmlOptions['id'], 'jQuery(' . $jqSelector . ').ckeditor(' . $ckeConfig . ');',  CClientScript::POS_END);
		} else {
			$selector = CJavaScript::encode($this -> inlineId);
			$jqSelector = CJavaScript::encode('#' . $this -> inlineId);
			Yii::app() -> clientScript -> registerScript('editMe_inline_' . $this -> inlineId, 'jQuery(' . $jqSelector . ').attr("contenteditable",true);CKEDITOR.disableAutoInline=true;CKEDITOR.inline(' . $selector . ',' . $ckeConfig . ');', CClientScript::POS_END);
		}
	}
